...
sample.all = sample[489:2635,]

# 回归准备-模型设定

## 对数收益率序列的弱平稳性检验
adfTest(sample.all[,1], lags = 10, type =c("c"), title = NULL, description =NULL) # type =c("nc", "c", "ct")
...
Box.test(at,lag=m, type='Ljung')        # Ljung-Box统计量,对数收益率的自相关,不显著则收益率不存在序列相关（仅存在弱序列相关）
