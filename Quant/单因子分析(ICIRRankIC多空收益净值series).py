...
def init(context):
    ...
    # 多组合收益率序列
    context.Equity_Long_series = [0]
    # 空组合收益率序列
    context.Equity_Short_series = [0]
    # 多空组合收益率序列
    context.Equity_LS_series = [0]
    # 序列的时间
...
def algo(context):
    ...
    # 选择因子属性
    factor_used = context.factor_list[7]
    factor_cut_used = context.factor_cut[factor_used]
    # factor_way_used = context.factor_way['ROIC']
    isTaFactor = False
    ...
    ## 当期因子计算
    ## ROCI ##
    fin = get_fundamentals(table='deriv_finance_indicator', symbols=not_suspended, start_date=last_day, end_date=last_day,
                           fields='ROIC', df=True)
    ...
    threshold = {}
    threshold[factor_used+'_SMALL'] = fin[factor_used].quantile(1/factor_cut_used)
    threshold[factor_used+'_BIG'] = fin[factor_used].quantile(1-1/factor_cut_used)
    ## 划分多空投资组合，记录收盘价共下期计算收益使用
    stock_bin = {}
    stock_bin[factor_used]={factor_used+'_SMALL':[],
                            factor_used+'_BIG':[]
                            }
    stock_bin[factor_used][factor_used+'_SMALL'] = list(set(fin[fin[factor_used]<threshold[factor_used+'_SMALL']].index))
    stock_bin[factor_used][factor_used+'_BIG'] = list(set(fin[fin[factor_used]>threshold[factor_used+'_BIG']].index))
    if context.period == 0:
        context.pre_small_close = now_close.reindex(stock_bin[factor_used][factor_used+'_SMALL'])
        context.pre_big_close = now_close.reindex(stock_bin[factor_used][factor_used+'_BIG'])
    else:
        pre_small_close = context.pre_small_close
        pre_big_close = context.pre_big_close
        ...
        context.Equity_Short_series.append(small_return_r)
        context.Equity_Long_series.append(big_return_r)
        context.Equity_LS_series.append(big_return_r-small_return_r)
...
def on_backtest_finished(context, indicator):
    ...
    # 计算净值序列
    Equity_Long_series_ = [i+1 for i in context.Equity_Long_series]
    Price_Long_series = []
    for i in range(len(Equity_Long_series_)):
        num = 1
        for j in Equity_Long_series_[:i+1]:
            num *= j
        Price_Long_series.append(num)
    Equity_Short_series_ = [i+1 for i in context.Equity_Short_series]
    Price_Short_series = []
    for i in range(len(Equity_Short_series_)):
        num = 1
        for j in Equity_Short_series_[:i+1]:
            num *= j
        Price_Short_series.append(num)
    Equity_LS_series_ = [i+1 for i in context.Equity_LS_series]
    Price_LS_series = []
    for i in range(len(Equity_LS_series_)):
        num = 1
        for j in Equity_LS_series_[:i+1]:
            num *= j
        Price_LS_series.append(num)

    save_dict = {'IC':context.IC_series,
                 'Rank_IC':context.Rank_IC_series,
                 'Long':context.Equity_Long_series,
                 'Short':context.Equity_Short_series,
                 'LS':context.Equity_LS_series,
    ...
    save_df.to_csv('C:/Users/jxjsj/Desktop/JupyterHome/Data/QuantOutPut/SingleFactorAnalysis'+factor_used+'.csv',index=True)
